...
def test_config(request):
    if request.param == 'single_language':
        return {
            'languages': ['nl'],
            'expected': {
                'char_filter': {
    ...
    if request.param == 'multiple_languages':
        return {
            'languages': ['nl', 'es'],
            'expected': {
                'char_filter': {
...
@pytest.mark.parametrize('test_config', ['single_language', 'multiple_languages'], indirect=True)
